...
describe("buildRouteEntries", () => {
  it("涵蓋固定頁面與每課的 lessons / quiz 路由", () => {
    ...
    for (const expected of ["/", "/lessons", "/review", "/stats", "/settings", "/quiz", "/lessons/1", "/lessons/2", "/quiz/1", "/quiz/2"]) {
  ...
  it("條目數 =(固定 6 頁 + 每課 2 頁)× 2,且全部使用指定 revision", () => {
    const entries = buildRouteEntries([1, 2, 3], "build-abc");

    expect(entries).toHaveLength((6 + 3 * 2) * 2);
    for (const entry of entries) {
